...
* I reran the UNet model and added extra metrics to the train_unet_with_openeds script. Previously, I was only able to view the average loss, but now I can begin recording proper segmentation metrics such as DICE, IoU etc which will be crucial for the benchmarking after training is done.
...
* Issues with training RIT-eyes on UNet still due to corruption supposedly, have redownloaded the dataset and unzipped on myriad and prepared a new job to diagnose if it is an issue with the dataset or augementations being applied.
